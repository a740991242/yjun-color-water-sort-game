...
export default function MocaipingGame() {
  ...
  // Persist selected mode to localStorage whenever it changes
  useEffect(() => {
  ...
  // Initialize game only when entering game screen
  useEffect(() => {
    if (screen !== 'game')
      return
  ...
  return (
    <>
      ...
      {/* Game Container — always mounted so ref is stable, hidden when not in game */}
      <div
        ref={hostRef}
